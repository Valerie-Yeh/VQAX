...
def load_checkpoint(ckpt_path, epoch):
    ...
    model_name = 'aokvqa/nle_model_{}'.format(str(epoch))
    tokenizer_name = 'aokvqa/nle_gpt2_tokenizer'
...
def save_checkpoint(epoch, unwrapped_model, optimizer, tokenizer, scheduler, ckpt_path, **kwargs):
    ...
    model_name = 'aokvqa/nle_model_{}'.format(str(epoch))
    tokenizer_name = 'aokvqa/nle_gpt2_tokenizer'.format(str(epoch))
    filename = 'aokvqa/ckpt_stats_' + str(epoch) + '.tar'
...
class AOKVQADataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        text_q = proc_ques(sample['question'])
        ...
        q_segment_id, a_segment_id, e_segment_id = self.tokenizer.convert_tokens_to_ids(['<question>', 
                                                                                         '<answer>', 
                                                                                         '<explanation>'])
        tokens = self.tokenizer.tokenize(text_q)
        ...
        tokens_e = self.tokenizer.tokenize(" because " + text_e) + [self.tokenizer.eos_token]
        exp_len = len(tokens_e)
        tokens += tokens_a + tokens_e
        labels += [-100] + tokens_a[1:] + tokens_e   # labels will be shifted in the model, so for now set them same as tokens
        ...
        folder = 'images/train2017/' if 'train' in self.path else 'images/val2017/' 
...
eval_batch_size = 1
img_size = 224
ckpt_path = 'finetuned_model/'
nle_data_train_path = 'nle_data/aokvqa/aokvqa_train.json'
nle_data_val_path = 'nle_data/aokvqa/aokvqa_val.json'
max_seq_len = 45
...
train_dataset = AOKVQADataset(path = nle_data_train_path,
...
val_dataset = AOKVQADataset(path = nle_data_val_path,
                            transform = img_transform,
                            tokenizer = tokenizer,
                            max_seq_len = max_seq_len)
...
for epoch in range(start_epoch, num_train_epochs):
    ...
    plt.savefig("ft_aokvqa_loss_woa.png")

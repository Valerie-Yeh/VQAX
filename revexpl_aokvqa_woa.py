...
def load_checkpoint(ckpt_path, epoch):
    ...
    model_name = 'aokvqa_woa/revexpl_model_{}'.format(str(epoch))
    tokenizer_name = 'aokvqa_woa/nle_gpt2_tokenizer'
    filename = 'aokvqa_woa/ckpt_stats_' + str(epoch) + '.tar'
...
class AOKVQADataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        q_segment_id, e_segment_id = self.tokenizer.convert_tokens_to_ids(['<question>',
                                                                           '<explanation>'])
        token_e = self.tokenizer.tokenize("because " + text_e)
        exp_len = len(token_e)
        tokens = token_e
        labels = [-100] * (exp_len) # we don't want to predict the answer and the explanation, set to pad to ignore
...
for epoch in range(start_epoch, num_train_epochs):
    ...
    plt.savefig("aokvqa_loss_woa.png")
